...
# Monkeypatch torch.load to default weights_only to False.
# PyTorch 2.6+ defaults weights_only=True, which blocks Stanza NLP weight deserialization.
...
def download_parser_jar_file():
    ...
    try:
        ...
        print(f"\nError downloading Stanford Parser: {e}")
        raise e
...
def extract_parser_jar_file():
    ...
    try:
        ...
        print(f"\nError extracting Stanford Parser jar file: {e}")
        if os.path.exists(stanford_parser_zip_file_path):
            try:
